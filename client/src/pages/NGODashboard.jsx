    ...
    <SidebarLayout links={LINKS} subLabel="NGO ADMIN PANEL" topRight={topRight}>
      <div className="page-enter">
        ...
        {activeTab === 'attendance' && (
          <div style={{ background:'white', borderRadius:12, border:'1px solid var(--gray-200)', padding:'18px 22px' }}>
            <div style={{ fontWeight:700, color:'var(--navy)', fontSize:'0.9rem', marginBottom:6 }}>Event Attendance</div>
            <p style={{ fontSize:'0.8rem', color:'var(--gray-500)', marginBottom:16 }}>Select an event to mark attendance for your volunteers.</p>
            {loading ? (
              <div style={{ textAlign:'center', padding:24 }}><span className="spinner" /></div>
            ) : events.length === 0 ? (
              <div style={{ textAlign:'center', padding:'32px 0', color:'var(--gray-400)' }}>
                <div style={{ fontSize:'2rem', marginBottom:8 }}>📋</div>
                <p>No events yet. Post an event first.</p>
              </div>
            ) : (
              <div style={{ display:'flex', flexDirection:'column', gap:10 }}>
                {events.map(ev => (
                  <div key={ev._id} style={{ border:'1px solid var(--gray-200)', borderRadius:9, padding:'14px 16px', display:'flex', alignItems:'center', gap:14 }}>
                    <div style={{ flex:1, minWidth:0 }}>
                      <div style={{ fontWeight:700, fontSize:'0.875rem', color:'var(--navy)', marginBottom:2 }}>{ev.title}</div>
                      <div style={{ fontSize:'0.75rem', color:'var(--gray-500)', display:'flex', gap:10 }}>
                        {ev.date && <span>📅 {new Date(ev.date).toLocaleDateString('en-IN', { day:'2-digit', month:'short', year:'numeric' })}</span>}
                        <span>👥 {ev.applicants?.length || 0} volunteer{ev.applicants?.length !== 1 ? 's' : ''}</span>
                      </div>
                    </div>
                    <Link
                      to={`/dashboard/attendance/${ev._id}`}
                      className="btn btn-primary btn-sm"
                      style={{ textDecoration:'none', display:'flex', alignItems:'center', gap:5, fontSize:'0.8rem' }}>
                      📋 Mark Attendance →
                    </Link>
                  </div>
                ))}
