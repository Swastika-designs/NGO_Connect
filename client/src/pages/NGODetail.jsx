...
export default function NGODetail() {
  ...
  const canDonate = !user || user.role === 'donor' || user.role === 'volunteer';
  ...
  // Pre-fill interest form with volunteer's skills/availability
  useEffect(() => {
  ...
  const handleInterestSubmit = async (e) => {
    ...
    try {
      ...
      toast.success('Interest submitted! The NGO will be in touch. ✅');
  ...
  return (
    <div className="page-enter" style={{ paddingTop:64, background:'var(--gray-50)', minHeight:'100vh' }}>
      {/* Cover */}
      <div style={{ height:240, background:ngo.coverImage ? `url(${ngo.coverImage}) center/cover` : `linear-gradient(135deg, ${catColor}22, ${catColor}55)`, position:'relative' }}>
      ...
      <div className="container" style={{ position:'relative', marginTop:-64 }}>
        ...
        <div style={{ background:'white', borderRadius:16, padding:'24px 28px', boxShadow:'var(--shadow-lg)', marginBottom:24, display:'flex', gap:20, alignItems:'flex-start', flexWrap:'wrap', border:'1px solid var(--gray-200)' }}>
          ...
          <div style={{ flex:1, minWidth:220 }}>
            <div style={{ display:'flex', gap:7, marginBottom:8, flexWrap:'wrap' }}>
              ...
              {ngo.isVerified && <span className="badge" style={{ background:'var(--green-pale)', color:'var(--green-dark)', border:'1px solid var(--green-light)' }}>✓ Verified</span>}
          ...
          <div style={{ display:'flex', gap:10, flexWrap:'wrap', flexShrink:0 }}>
            {/* Donate: only for non-ngo, non-admin users */}
            {canDonate && (
              <button className="btn btn-primary" onClick={() => setDonateOpen(true)} style={{ padding:'12px 28px' }}>
                💝 Donate Now
              </button>
            )}
            {/* Volunteer interest: only for volunteer role */}
            {isVolunteer && (
              interestSubmitted ? (
                <span className="badge badge-green" style={{ padding:'10px 16px', fontSize:'0.85rem' }}>✓ Interest Submitted</span>
              ) : (
        ...
        <div style={{ display:'grid', gridTemplateColumns:'1fr 320px', gap:22, alignItems:'start' }}>
          <div>
            ...
            {/* Donors — only shown when not ngo/admin */}
            {!isNgoOrAdmin && (
          ...
          <div style={{ display:'flex', flexDirection:'column', gap:16 }}>
            ...
            <div className="card" style={{ padding:'18px 20px' }}>
              ...
              {[
                ...
                ngo.volunteerCount && ['Volunteers', ngo.volunteerCount],
            ...
            {/* CTA buttons in sidebar — role-aware */}
            {canDonate && (
              <button className="btn btn-primary" onClick={() => setDonateOpen(true)} style={{ width:'100%', justifyContent:'center', padding:14 }}>
                💝 Support This NGO
              </button>
            )}
            {isVolunteer && !interestSubmitted && (
              <button className="btn btn-outline" onClick={() => setInterestOpen(true)} style={{ width:'100%', justifyContent:'center', padding:14 }}>
                ✋ Apply to Volunteer
              </button>
            )}
            {isNgoOrAdmin && (
              <div style={{ background:'var(--gray-100)', borderRadius:10, padding:'14px 16px', textAlign:'center' }}>
      ...
      {/* Donate Modal — guarded internally for ngo/admin */}
            ...
            <form onSubmit={handleInterestSubmit} style={{ padding:24 }}>
              <div className="form-group">
                ...
                <textarea
                  ...
                  placeholder="Tell the NGO about your motivation and what you hope to contribute..."
                ...
                <div style={{ display:'flex', flexWrap:'wrap', gap:8 }}>
                  {ALL_SKILLS.map(s => (
                    <button key={s} type="button" onClick={() => toggleSkill(s)} style={{
                      ...
                      {interestForm.skills.includes(s) ? '✓ ' : ''}{s}
                    </button>
                  ))}
                </div>
              </div>
