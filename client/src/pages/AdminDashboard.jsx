...
const FEEDBACK = [
  { n:'Sarah Jenkins', t:'2H AGO', tx:'The new donation process is so much smoother now. Thank you!', pos:true },
  { n:'Marcus Thorne', t:'5H AGO', tx:"Still waiting for my NGO verification. It's been 5 days.", pos:false },
  { n:'Elena Rodriguez', t:'1D AGO', tx:'Great platform! Would love to see a map view of active NGOs soon.', pos:true },
  { n:'David Chen', t:'1D AGO', tx:'The volunteer matching feature is incredible.', pos:true },
];
...
export default function AdminDashboard() {
  ...
  useEffect(() => {
    Promise.all([ngoAPI.getPending(), statsAPI.get()])
      .then(([n, s]) => {
        ...
        <div style={{ display:'grid', gridTemplateColumns:'1fr 340px', gap:18 }}>
          ...
          <div style={{ background:'white', borderRadius:12, border:'1px solid var(--gray-200)', padding:'18px 20px' }}>
            <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:14 }}>
              ...
              <button style={{ fontSize:'0.68rem', fontWeight:700, color:'var(--gray-400)', background:'none', border:'none', cursor:'pointer', letterSpacing:'0.05em' }}>MODERATION VIEW</button>
            </div>
            {FEEDBACK.map((f, i) => (
              <div key={i} style={{ paddingBottom:12, marginBottom:12, borderBottom:i < FEEDBACK.length-1 ? '1px solid var(--gray-100)' : 'none' }}>
                <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:4 }}>
                  <div><span style={{ fontWeight:700, fontSize:'0.85rem', color:'var(--navy)' }}>{f.n}</span><span style={{ fontSize:'0.7rem', color:'var(--gray-400)', marginLeft:6 }}>• {f.t}</span></div>
                  <div style={{ width:34, height:19, borderRadius:100, cursor:'pointer', background:f.pos ? 'var(--green)' : 'var(--gray-300)', position:'relative' }}>
                    <div style={{ width:15, height:15, borderRadius:'50%', background:'white', position:'absolute', top:2, left:f.pos ? 17 : 2, transition:'left 0.2s', boxShadow:'0 1px 2px rgba(0,0,0,0.2)' }} />
                  </div>
                </div>
                <p style={{ fontSize:'0.78rem', color:'var(--gray-600)', lineHeight:1.6 }}>{f.tx}</p>
              </div>
            ))}
          </div>
        </div>
