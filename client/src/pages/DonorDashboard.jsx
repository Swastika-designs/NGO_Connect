      ...
      <div className="page-enter">
        ...
        <div style={{ background:'white', borderRadius:12, border:'1px solid var(--gray-200)', padding:'18px 22px' }}>
          ...
          <table className="table">
            <thead><tr><th>NGO Name</th><th>Date</th><th>Type</th><th>Details</th><th>Status</th></tr></thead>
            <tbody>
              {loading ? (
                ...
                <tr key={d._id}>
                  ...
                  <td>
                    <span style={{ fontSize:'0.75rem', fontWeight:700, padding:'3px 8px', borderRadius:100, background: d.donationType==='goods'?'#EFF6FF':'var(--green-pale)', color: d.donationType==='goods'?'#1D4ED8':'var(--green)' }}>
                      {d.donationType==='goods' ? '📦 Goods' : '💳 Money'}
                    </span>
                  </td>
                  <td style={{ fontWeight:700, fontSize:'0.85rem' }}>
                    {d.donationType==='goods'
                      ? <span style={{ fontSize:'0.8rem', color:'var(--gray-600)' }}>{d.goodsCategory} · {d.goodsDescription?.slice(0,30)}{d.goodsDescription?.length>30?'...':''}</span>
