...
export default function VolunteerDashboard() {
  ...
  const [cantAttendId, setCantAttendId] = useState(null);
  const [cantAttendMsg, setCantAttendMsg] = useState('');
  ...
  const handleCantAttend = async (ev) => {
    try {
      const ngoId = ev.createdBy?._id || ev.ngo;
      if (ngoId) {
        // send a message to the NGO notifying can't attend
        await import('../services/api').then(m => m.messageAPI?.send?.({
          to: ev.createdBy?._id,
          subject: `Cannot attend: ${ev.title}`,
          content: cantAttendMsg || `I wanted to let you know that I won't be able to attend "${ev.title}". I apologize for any inconvenience.`,
        })).catch(() => {});
      }
      toast.success('NGO has been notified that you cannot attend ✅');
      setCantAttendId(null);
      setCantAttendMsg('');
    } catch {
                  ...
                  <div style={{ display:'flex', flexDirection:'column', gap:10 }}>
                    {events.map(ev => (
                      <div key={ev._id} style={{ border:'1px solid var(--gray-200)', borderRadius:9, overflow:'hidden' }}>
                        <div style={{ height:5, background: CAT_BG[ev.category] ? 'var(--green)' : 'var(--gray-300)' }} />
                        <div style={{ padding:'12px 14px', display:'flex', alignItems:'center', gap:12 }}>
                          <div style={{ flex:1, minWidth:0 }}>
                            <div style={{ fontWeight:700, fontSize:'0.875rem', color:'var(--navy)', marginBottom:2 }}>{ev.title}</div>
                            <div style={{ fontSize:'0.72rem', color:'var(--gray-500)', display:'flex', gap:8, flexWrap:'wrap' }}>
                              <span>{ev.createdBy?.name || 'NGO'}</span>
                              {ev.date && <span>📅 {new Date(ev.date).toLocaleDateString('en-IN', { weekday:'short', day:'2-digit', month:'short' })}</span>}
                              {ev.time && <span>🕐 {ev.time}</span>}
                              {ev.location && <span>📍 {ev.location}</span>}
                            </div>
                            {ev.description && (
                              <p style={{ fontSize:'0.77rem', color:'var(--gray-500)', marginTop:5, lineHeight:1.5, display:'-webkit-box', WebkitLineClamp:2, WebkitBoxOrient:'vertical', overflow:'hidden' }}>
                                {ev.description}
                              </p>
                            )}
                            {(ev.requiredSkills||[]).length > 0 && (
                              <div style={{ display:'flex', gap:4, marginTop:5, flexWrap:'wrap' }}>
                                {ev.requiredSkills.slice(0,3).map(s => (
                          ...
                          <div style={{ display:'flex', flexDirection:'column', gap:6, flexShrink:0, alignItems:'flex-end' }}>
                            {hasApplied(ev) ? (
                              <>
                                ...
                                {cantAttendId !== ev._id ? (
                                  <button style={{ fontSize:'0.7rem', color:'var(--red)', background:'none', border:'1px solid var(--red)', borderRadius:5, padding:'3px 9px', cursor:'pointer', fontWeight:600 }}
                                    onClick={() => setCantAttendId(ev._id)}>
                                    Can't Attend
                                  </button>
                                ) : (
                                  <div style={{ background:'#FEF3C7', border:'1px solid #FCD34D', borderRadius:7, padding:'8px 10px', width:200 }}>
                                    <div style={{ fontSize:'0.72rem', fontWeight:700, color:'#92400E', marginBottom:5 }}>⚠️ Notify NGO</div>
                                    <textarea style={{ width:'100%', fontSize:'0.72rem', borderRadius:5, border:'1px solid var(--gray-200)', padding:'5px 7px', resize:'none', marginBottom:5, boxSizing:'border-box' }}
                                      rows={2} placeholder="Reason (optional)..."
                                      value={cantAttendMsg} onChange={e => setCantAttendMsg(e.target.value)} />
                                    <div style={{ display:'flex', gap:5 }}>
                                      <button style={{ flex:1, fontSize:'0.68rem', padding:'4px', background:'var(--red)', color:'white', border:'none', borderRadius:4, cursor:'pointer', fontWeight:700 }}
                                        onClick={() => handleCantAttend(ev)}>
                                        Send Notice
                                      </button>
                                      <button style={{ flex:1, fontSize:'0.68rem', padding:'4px', background:'var(--gray-100)', border:'none', borderRadius:4, cursor:'pointer' }}
                                        onClick={() => { setCantAttendId(null); setCantAttendMsg(''); }}>
                                        Cancel
                                      </button>
                                    </div>
                                  </div>
                              ...
                              <button className="btn btn-dark btn-sm" style={{ fontSize:'0.7rem', padding:'6px 12px' }}
                                disabled={applying === ev._id} onClick={() => handleApply(ev._id)}>
                                {applying === ev._id ? '...' : '✋ Apply'}
                              </button>
                            )}
                          </div>
                        </div>
